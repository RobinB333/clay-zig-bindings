...
typedef struct Clay_LayoutConfig {
    Clay_Sizing sizing;            // unchanged
    Clay_Padding padding;          // unchanged

    // Gap between children on the layout axis (same as before)
    uint16_t childGap;

    // NEW: gap between wrapped rows/columns (cross axis)
    uint16_t wrapChildGap;

    Clay_ChildAlignment childAlignment;     // unchanged
    Clay_LayoutDirection layoutDirection;   // unchanged

    // NEW: enable/disable wrapping of children to new row/column
    bool wrapChildren;
} Clay_LayoutConfig;
...
void Clay__SizeContainersAlongAxis(bool xAxis) {
    Clay_Context* context = Clay_GetCurrentContext();
    Clay__int32_tArray bfsQueue = context->layoutElementChildrenBuffer;
    Clay__int32_tArray resizableChildrenBuffer = context->openLayoutElementStack;

    for (int32_t rootTreeIndex = 0; rootTreeIndex < context->layoutElementTreeRoots.length; ++rootTreeIndex) {
        bfsQueue.length = 0;

        Clay__LayoutElementTreeRoot *rootTree = Clay__LayoutElementTreeRootArray_Get(&context->layoutElementTreeRoots, rootTreeIndex);
        Clay_LayoutElement *rootElement = Clay_LayoutElementArray_Get(&context->layoutElements, (int)rootTree->layoutElementIndex);
        Clay__int32_tArray_Add(&bfsQueue, (int32_t)rootTree->layoutElementIndex);
        ...
        for (int32_t bfsIndex = 0; bfsIndex < bfsQueue.length; ++bfsIndex) {
            int32_t parentElementIndex = Clay__int32_tArray_GetValue(&bfsQueue, bfsIndex);
            Clay_LayoutElement *parentElement = Clay_LayoutElementArray_Get(&context->layoutElements, parentElementIndex);
            Clay_LayoutConfig *parentLayoutConfig = parentElement->layoutConfig;

            int32_t growChildCount = 0;

            float parentSizeAlongAxis = xAxis ? parentElement->dimensions.width : parentElement->dimensions.height;
            float parentPaddingAlongAxis = (float)(
                xAxis
            ...
            float innerContentSize = 0.0f;
            float totalPaddingAndGaps = parentPaddingAlongAxis;

            bool isSizingAlongLayoutAxis =
                (xAxis && parentLayoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) ||
                (!xAxis && parentLayoutConfig->layoutDirection == CLAY_TOP_TO_BOTTOM);

            // WRAP FIX: if we're sizing along the layout axis and wrapping is enabled on this parent,
            // we must not "compress-to-fit" children when content is too large. Wrapping handles overflow.
            bool parentWrapsChildren = (isSizingAlongLayoutAxis && parentLayoutConfig->wrapChildren);

            resizableChildrenBuffer.length = 0;
            float parentChildGap = (float)parentLayoutConfig->childGap;

            // Scan children: enqueue containers, collect resizables, compute content size, count GROW
            for (int32_t childOffset = 0; childOffset < parentElement->childrenOrTextContent.children.length; ++childOffset) {
                int32_t childElementIndex = parentElement->childrenOrTextContent.children.elements[childOffset];
                Clay_LayoutElement *childElement = Clay_LayoutElementArray_Get(&context->layoutElements, childElementIndex);

                Clay_SizingAxis childSizingAxis = xAxis ? childElement->layoutConfig->sizing.width : childElement->layoutConfig->sizing.height;
                float childCurrentSize = xAxis ? childElement->dimensions.width : childElement->dimensions.height;
                ...
                if (!Clay__ElementHasConfig(childElement, CLAY__ELEMENT_CONFIG_TYPE_TEXT) && childElement->childrenOrTextContent.children.length > 0) {
                    Clay__int32_tArray_Add(&bfsQueue, childElementIndex);
                }

                if (childSizingAxis.type != CLAY__SIZING_TYPE_PERCENT
                    && childSizingAxis.type != CLAY__SIZING_TYPE_FIXED
                    ...
                    Clay__int32_tArray_Add(&resizableChildrenBuffer, childElementIndex);
                }

                if (isSizingAlongLayoutAxis) {
                    innerContentSize += (childSizingAxis.type == CLAY__SIZING_TYPE_PERCENT ? 0.0f : childCurrentSize);

                    if (childSizingAxis.type == CLAY__SIZING_TYPE_GROW) {
                        growChildCount++;
                    }

                    if (childOffset > 0) {
                        innerContentSize += parentChildGap;
                        totalPaddingAndGaps += parentChildGap;
                    }
                } else {
                    innerContentSize = CLAY__MAX(childCurrentSize, innerContentSize);
                }
            }

            // Expand percentage children
            for (int32_t childOffset = 0; childOffset < parentElement->childrenOrTextContent.children.length; ++childOffset) {
                int32_t childElementIndex = parentElement->childrenOrTextContent.children.elements[childOffset];
                Clay_LayoutElement *childElement = Clay_LayoutElementArray_Get(&context->layoutElements, childElementIndex);

                Clay_SizingAxis childSizingAxis = xAxis ? childElement->layoutConfig->sizing.width : childElement->layoutConfig->sizing.height;
                float *childSizePtr = xAxis ? &childElement->dimensions.width : &childElement->dimensions.height;

                if (childSizingAxis.type == CLAY__SIZING_TYPE_PERCENT) {
                    *childSizePtr = (parentSizeAlongAxis - totalPaddingAndGaps) * childSizingAxis.size.percent;
                    if (isSizingAlongLayoutAxis) {
                        innerContentSize += *childSizePtr;
                    }
            ...
            if (isSizingAlongLayoutAxis) {
                float sizeToDistribute = parentSizeAlongAxis - parentPaddingAlongAxis - innerContentSize;

                // Content too large
                if (sizeToDistribute < 0.0f) {
                    // WRAP FIX: do not compress children in wrap containers; wrapping will handle overflow.
                    if (parentWrapsChildren) {
                        continue;
                    }

                    // Original behavior: if clipped in this axis, don't compress
                    Clay_ClipElementConfig *clipElementConfig =
                        Clay__FindElementConfigWithType(parentElement, CLAY__ELEMENT_CONFIG_TYPE_CLIP).clipElementConfig;

                    if (clipElementConfig) {
                        bool clipsThisAxis = (xAxis && clipElementConfig->horizontal) || (!xAxis && clipElementConfig->vertical);
                        if (clipsThisAxis) {
                            continue;
                        }
                    }

                    // Compress children (original)
                    while (sizeToDistribute < -CLAY__EPSILON && resizableChildrenBuffer.length > 0) {
                        float largestSize = 0.0f;
                        float secondLargestSize = 0.0f;
                        float deltaToApply = sizeToDistribute;

                        for (int32_t bufferIndex = 0; bufferIndex < resizableChildrenBuffer.length; ++bufferIndex) {
                            int32_t childElementIndex = Clay__int32_tArray_GetValue(&resizableChildrenBuffer, bufferIndex);
                            Clay_LayoutElement *childElement = Clay_LayoutElementArray_Get(&context->layoutElements, childElementIndex);

                            float childSize = xAxis ? childElement->dimensions.width : childElement->dimensions.height;
                            if (Clay__FloatEqual(childSize, largestSize)) {
                                continue;
                            }
                            ...
                            if (childSize < largestSize) {
                                secondLargestSize = CLAY__MAX(secondLargestSize, childSize);
                                deltaToApply = secondLargestSize - largestSize;
                            }
                        }

                        deltaToApply = CLAY__MAX(deltaToApply, sizeToDistribute / (float)resizableChildrenBuffer.length);
                        ...
                        for (int32_t bufferIndex = 0; bufferIndex < resizableChildrenBuffer.length; ++bufferIndex) {
                            ...
                            float previousSize = *childSizePtr;
                            if (Clay__FloatEqual(*childSizePtr, largestSize)) {
                                *childSizePtr += deltaToApply;
                                if (*childSizePtr <= childMinSize) {
                                    *childSizePtr = childMinSize;
                                    Clay__int32_tArray_RemoveSwapback(&resizableChildrenBuffer, bufferIndex--);
                                }
                                sizeToDistribute -= (*childSizePtr - previousSize);
                            }
                        }
                    }
                }
                // Content too small, distribute into GROW children (original)
                else if (sizeToDistribute > 0.0f && growChildCount > 0) {
                    for (int32_t bufferIndex = 0; bufferIndex < resizableChildrenBuffer.length; ++bufferIndex) {
                        int32_t childElementIndex = Clay__int32_tArray_GetValue(&resizableChildrenBuffer, bufferIndex);
                        Clay_LayoutElement *childElement = Clay_LayoutElementArray_Get(&context->layoutElements, childElementIndex);

                        Clay__SizingType childSizingType = xAxis ? childElement->layoutConfig->sizing.width.type : childElement->layoutConfig->sizing.height.type;
                        if (childSizingType != CLAY__SIZING_TYPE_GROW) {
                            Clay__int32_tArray_RemoveSwapback(&resizableChildrenBuffer, bufferIndex--);
                        }
                    }

                    while (sizeToDistribute > CLAY__EPSILON && resizableChildrenBuffer.length > 0) {
                        float smallestSize = CLAY__MAXFLOAT;
                        float secondSmallestSize = CLAY__MAXFLOAT;
                        float deltaToApply = sizeToDistribute;
                        ...
                        for (int32_t bufferIndex = 0; bufferIndex < resizableChildrenBuffer.length; ++bufferIndex) {
                            ...
                            float childSize = xAxis ? childElement->dimensions.width : childElement->dimensions.height;
                            if (Clay__FloatEqual(childSize, smallestSize)) {
                                continue;
                            }
                            ...
                            if (childSize > smallestSize) {
                                secondSmallestSize = CLAY__MIN(secondSmallestSize, childSize);
                                deltaToApply = secondSmallestSize - smallestSize;
                            }
                        }

                        deltaToApply = CLAY__MIN(deltaToApply, sizeToDistribute / (float)resizableChildrenBuffer.length);
                        ...
                        for (int32_t bufferIndex = 0; bufferIndex < resizableChildrenBuffer.length; ++bufferIndex) {
                            ...
                            float *childSizePtr = xAxis ? &childElement->dimensions.width : &childElement->dimensions.height;
                            float childMaxSize = xAxis ? childElement->layoutConfig->sizing.width.size.minMax.max : childElement->layoutConfig->sizing.height.size.minMax.max;

                            float previousSize = *childSizePtr;
                            if (Clay__FloatEqual(*childSizePtr, smallestSize)) {
                                *childSizePtr += deltaToApply;
                                if (*childSizePtr >= childMaxSize) {
                                    *childSizePtr = childMaxSize;
                                    Clay__int32_tArray_RemoveSwapback(&resizableChildrenBuffer, bufferIndex--);
                                }
                                sizeToDistribute -= (*childSizePtr - previousSize);
                            }
            ...
            // Off-axis sizing (original)
                ...
                for (int32_t bufferIndex = 0; bufferIndex < resizableChildrenBuffer.length; ++bufferIndex) {
                    ...
                    float maxSize = parentSizeAlongAxis - parentPaddingAlongAxis;

                    if (Clay__ElementHasConfig(parentElement, CLAY__ELEMENT_CONFIG_TYPE_CLIP)) {
                        Clay_ClipElementConfig *clipElementConfig =
                            Clay__FindElementConfigWithType(parentElement, CLAY__ELEMENT_CONFIG_TYPE_CLIP).clipElementConfig;
                    ...
                    if (childSizingAxis.type == CLAY__SIZING_TYPE_GROW) {
                        float childMaxSize = xAxis ? childElement->layoutConfig->sizing.width.size.minMax.max : childElement->layoutConfig->sizing.height.size.minMax.max;
                        *childSizePtr = CLAY__MIN(maxSize, childMaxSize);
                    }

                    *childSizePtr = CLAY__MAX(childMinSize, CLAY__MIN(*childSizePtr, maxSize));
                }
            }
        }
    }
...
static Clay_Dimensions Clay__ComputeChildLayoutContentSize(Clay_Context* context, Clay_LayoutElement* parent) {
    (void)context;

    Clay_LayoutConfig* layoutConfig = parent->layoutConfig;
    int32_t childCount = parent->childrenOrTextContent.children.length;
    ...
    bool isLeftToRight = (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT);

    float childGap = (float)layoutConfig->childGap;
    float wrapGap = (float)layoutConfig->wrapChildGap;

    float paddingMain = isLeftToRight
        ? (float)(layoutConfig->padding.left + layoutConfig->padding.right)
        : (float)(layoutConfig->padding.top + layoutConfig->padding.bottom);

    float parentInnerMainLimit = isLeftToRight
        ? (parent->dimensions.width - paddingMain)
        : (parent->dimensions.height - paddingMain);

    bool wrapActive = (layoutConfig->wrapChildren && parentInnerMainLimit > 0.0f);
    ...
    if (!wrapActive) {
        if (isLeftToRight) {
            ...
            for (int32_t childOffset = 0; childOffset < childCount; ++childOffset) {
                int32_t childElementIndex = parent->childrenOrTextContent.children.elements[childOffset];
                Clay_LayoutElement* child = Clay_LayoutElementArray_Get(&Clay_GetCurrentContext()->layoutElements, childElementIndex);
                totalWidth += child->dimensions.width;
                maxHeight = CLAY__MAX(maxHeight, child->dimensions.height);
            }
            totalWidth += (float)(CLAY__MAX(childCount - 1, 0) * layoutConfig->childGap);
            ...
            for (int32_t childOffset = 0; childOffset < childCount; ++childOffset) {
                int32_t childElementIndex = parent->childrenOrTextContent.children.elements[childOffset];
                Clay_LayoutElement* child = Clay_LayoutElementArray_Get(&Clay_GetCurrentContext()->layoutElements, childElementIndex);
                maxWidth = CLAY__MAX(maxWidth, child->dimensions.width);
                totalHeight += child->dimensions.height;
            }
            totalHeight += (float)(CLAY__MAX(childCount - 1, 0) * layoutConfig->childGap);
    ...
    // Wrapping content-size (max line main-size, sum of line cross-sizes)
    float contentMainMax = 0.0f;
    float contentCrossSum = 0.0f;

    int32_t childOffset = 0;
    while (childOffset < childCount) {
        float lineMainUsed = 0.0f;
        float lineCrossMax = 0.0f;
        int32_t lineStart = childOffset;
        int32_t lineCount = 0;
        ...
        while (childOffset < childCount) {
            ...
            float childMain = isLeftToRight ? child->dimensions.width : child->dimensions.height;
            float childCross = isLeftToRight ? child->dimensions.height : child->dimensions.width;

            Clay__SizingType childMainSizingType = isLeftToRight
                ? child->layoutConfig->sizing.width.type
                : child->layoutConfig->sizing.height.type;

            bool childIsGrow = (childMainSizingType == CLAY__SIZING_TYPE_GROW);

            float gapBefore = (lineCount > 0) ? childGap : 0.0f;
            float tentativeMain = lineMainUsed + gapBefore + childMain;

            bool wouldOverflow =
                (!childIsGrow) &&
                (lineCount > 0) &&
                (tentativeMain > parentInnerMainLimit + CLAY__EPSILON);

            if (wouldOverflow) {
                break;
            }

            lineMainUsed = tentativeMain;
            lineCrossMax = CLAY__MAX(lineCrossMax, childCross);
            lineCount++;
            childOffset++;
        }
        ...
        contentMainMax = CLAY__MAX(contentMainMax, lineMainUsed);
        contentCrossSum += lineCrossMax;

        if (childOffset < childCount) {
            contentCrossSum += wrapGap;
        }
    ...
    if (isLeftToRight) {
        contentSize.width = contentMainMax;
        contentSize.height = contentCrossSum;
    } else {
        contentSize.width = contentCrossSum;
        contentSize.height = contentMainMax;
    }

    return contentSize;
}

static void Clay__PushChildrenToDfsBuffer_WithOptionalWrap(
    Clay_Context* context,
    Clay__LayoutElementTreeNodeArray* dfsBuffer,
    Clay__LayoutElementTreeNode* parentTreeNode,
    Clay_LayoutElement* parentElement,
    Clay_Vector2 scrollOffset
) {
    Clay_LayoutConfig* layoutConfig = parentElement->layoutConfig;
    int32_t childCount = parentElement->childrenOrTextContent.children.length;
    if (childCount <= 0) {
        return;
    }
    ...
    float parentInnerMainLimit = isLeftToRight
        ? (parentElement->dimensions.width - paddingMain)
        : (parentElement->dimensions.height - paddingMain);

    bool wrapActive = (layoutConfig->wrapChildren && parentInnerMainLimit > 0.0f);

    // Extend DFS buffer for children (same shape as original)
    dfsBuffer->length += childCount;

    // If wrap is not active, use the original placement logic verbatim (but written here as a full block).
    if (!wrapActive) {
        for (int32_t childOffset = 0; childOffset < childCount; ++childOffset) {
            Clay_LayoutElement* childElement =
                Clay_LayoutElementArray_Get(&context->layoutElements, parentElement->childrenOrTextContent.children.elements[childOffset]);

            // Alignment along non layout axis (original behavior)
            if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
                parentTreeNode->nextChildOffset.y = (float)parentElement->layoutConfig->padding.top;
                float whiteSpaceAroundChild =
                    parentElement->dimensions.height -
                    (float)(layoutConfig->padding.top + layoutConfig->padding.bottom) -
                    childElement->dimensions.height;

                switch (layoutConfig->childAlignment.y) {
                    case CLAY_ALIGN_Y_TOP: break;
                    case CLAY_ALIGN_Y_CENTER: parentTreeNode->nextChildOffset.y += whiteSpaceAroundChild / 2.0f; break;
                    case CLAY_ALIGN_Y_BOTTOM: parentTreeNode->nextChildOffset.y += whiteSpaceAroundChild; break;
                }
            } else {
                parentTreeNode->nextChildOffset.x = (float)parentElement->layoutConfig->padding.left;
                float whiteSpaceAroundChild =
                    parentElement->dimensions.width -
                    (float)(layoutConfig->padding.left + layoutConfig->padding.right) -
                    childElement->dimensions.width;

                switch (layoutConfig->childAlignment.x) {
                    case CLAY_ALIGN_X_LEFT: break;
                    case CLAY_ALIGN_X_CENTER: parentTreeNode->nextChildOffset.x += whiteSpaceAroundChild / 2.0f; break;
                    case CLAY_ALIGN_X_RIGHT: parentTreeNode->nextChildOffset.x += whiteSpaceAroundChild; break;
                }
            }

            Clay_Vector2 childPosition = {
                parentTreeNode->position.x + parentTreeNode->nextChildOffset.x + scrollOffset.x,
                parentTreeNode->position.y + parentTreeNode->nextChildOffset.y + scrollOffset.y,
            };

            // DFS buffer elements need to be added in reverse because stack traversal happens backwards
            uint32_t newNodeIndex = (uint32_t)(dfsBuffer->length - 1 - childOffset);
            ...
            // Update parent offsets (original)
            if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
                parentTreeNode->nextChildOffset.x += childElement->dimensions.width + (float)layoutConfig->childGap;
            } else {
                parentTreeNode->nextChildOffset.y += childElement->dimensions.height + (float)layoutConfig->childGap;
            }
        }
        return;
    }
    ...
    float crossCursor = 0.0f;  // offset into cross axis (relative to cross padding)

    int32_t childOffset = 0;
    while (childOffset < childCount) {
        // First pass: determine line/column extent
        int32_t lineStart = childOffset;
        int32_t lineCount = 0;
        float lineMainUsed = 0.0f;
        float lineCrossMax = 0.0f;
        ...
        // Per-line main-axis alignment (childAlignment.x for LTR, childAlignment.y for TTB)
        float extraSpace = CLAY__MAX(parentInnerMainLimit - lineMainUsed, 0.0f);
        float lineMainAlignmentOffset = 0.0f;

        if (isLeftToRight) {
            switch (layoutConfig->childAlignment.x) {
                case CLAY_ALIGN_X_LEFT:   lineMainAlignmentOffset = 0.0f; break;
                case CLAY_ALIGN_X_CENTER: lineMainAlignmentOffset = extraSpace / 2.0f; break;
                case CLAY_ALIGN_X_RIGHT:  lineMainAlignmentOffset = extraSpace; break;
            }
        } else {
            switch (layoutConfig->childAlignment.y) {
                case CLAY_ALIGN_Y_TOP:    lineMainAlignmentOffset = 0.0f; break;
                case CLAY_ALIGN_Y_CENTER: lineMainAlignmentOffset = extraSpace / 2.0f; break;
                case CLAY_ALIGN_Y_BOTTOM: lineMainAlignmentOffset = extraSpace; break;
            }
        }

        // Second pass: assign child nodes with final positions
        float mainCursor = 0.0f;

        for (int32_t lineChildIndex = 0; lineChildIndex < lineCount; ++lineChildIndex) {
            int32_t childIndexInParent = lineStart + lineChildIndex;
            int32_t childElementIndex = parentElement->childrenOrTextContent.children.elements[childIndexInParent];
            Clay_LayoutElement* child = Clay_LayoutElementArray_Get(&context->layoutElements, childElementIndex);
            ...
            float crossWhiteSpace = CLAY__MAX(lineCrossMax - childCross, 0.0f);
            float crossAlignmentOffset = 0.0f;

            if (isLeftToRight) {
                switch (layoutConfig->childAlignment.y) {
                    case CLAY_ALIGN_Y_TOP:    crossAlignmentOffset = 0.0f; break;
                    case CLAY_ALIGN_Y_CENTER: crossAlignmentOffset = crossWhiteSpace / 2.0f; break;
                    case CLAY_ALIGN_Y_BOTTOM: crossAlignmentOffset = crossWhiteSpace; break;
                }
            } else {
                switch (layoutConfig->childAlignment.x) {
                    case CLAY_ALIGN_X_LEFT:   crossAlignmentOffset = 0.0f; break;
                    case CLAY_ALIGN_X_CENTER: crossAlignmentOffset = crossWhiteSpace / 2.0f; break;
                    case CLAY_ALIGN_X_RIGHT:  crossAlignmentOffset = crossWhiteSpace; break;
                }
            }

            Clay_Vector2 childPosition = {0, 0};

            if (isLeftToRight) {
                float startX = (float)layoutConfig->padding.left + lineMainAlignmentOffset;
                float startY = (float)layoutConfig->padding.top + crossCursor;

                childPosition.x = parentTreeNode->position.x + scrollOffset.x + startX + mainCursor;
                childPosition.y = parentTreeNode->position.y + scrollOffset.y + startY + crossAlignmentOffset;
            } else {
                float startX = (float)layoutConfig->padding.left + crossCursor;
                float startY = (float)layoutConfig->padding.top + lineMainAlignmentOffset;

                childPosition.x = parentTreeNode->position.x + scrollOffset.x + startX + crossAlignmentOffset;
                childPosition.y = parentTreeNode->position.y + scrollOffset.y + startY + mainCursor;
            }

            uint32_t newNodeIndex = (uint32_t)(dfsBuffer->length - 1 - childIndexInParent);
            dfsBuffer->internalArray[newNodeIndex] = CLAY__INIT(Clay__LayoutElementTreeNode) {
                .layoutElement = child,
                .position = { childPosition.x, childPosition.y },
                .nextChildOffset = { .x = (float)child->layoutConfig->padding.left, .y = (float)child->layoutConfig->padding.top },
            };
            context->treeNodeVisited.internalArray[newNodeIndex] = false;
        ...
        // Move to next wrapped row/column
        crossCursor += lineCrossMax;
        if (childOffset < childCount) {
            crossCursor += wrapGap;
        }
    }
...
// ============================
// 3) REPLACE THIS ENTIRE FUNCTION (full copy/paste)
// ============================
...
void Clay__CalculateFinalLayout(void) {
    ...
    // Propagate effect of text wrapping, aspect scaling etc. on height of parents (unchanged)
    ...
    for (int32_t rootIndex = 0; rootIndex < context->layoutElementTreeRoots.length; ++rootIndex) {
        ...
        // Position root floating containers (unchanged)
        ...
        while (dfsBuffer.length > 0) {
            ...
            if (!context->treeNodeVisited.internalArray[dfsBuffer.length - 1]) {
                ...
                // Setup initial on-axis alignment + scroll content size
                if (!Clay__ElementHasConfig(currentElementTreeNode->layoutElement, CLAY__ELEMENT_CONFIG_TYPE_TEXT)) {
                    Clay_Dimensions contentSize = Clay__ComputeChildLayoutContentSize(context, currentElement);
                    ...
                    // Only apply the original "global" main-axis alignment when wrapping is NOT active.
                    bool isLeftToRight = (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT);
                    float paddingMain = isLeftToRight
                        ? (float)(layoutConfig->padding.left + layoutConfig->padding.right)
                        : (float)(layoutConfig->padding.top + layoutConfig->padding.bottom);
                    ...
                    float parentInnerMainLimit = isLeftToRight
                        ? (currentElement->dimensions.width - paddingMain)
                        : (currentElement->dimensions.height - paddingMain);
                ...
                if (Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_BORDER)) {
                    ...
                    if (!Clay__ElementIsOffscreen(&currentElementBoundingBox)) {
                        Clay_SharedElementConfig *sharedConfig = Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_SHARED)
                        ...
                        // IMPORTANT: betweenChildren borders are ambiguous under wrap, so skip them when wrapChildren == true.
                        if (!layoutConfig->wrapChildren && borderConfig->width.betweenChildren > 0 && borderConfig->color.a > 0) {
            ...
            // Add children to the DFS buffer (UPDATED: optional wrap)
            if (!Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_TEXT)) {
                Clay__PushChildrenToDfsBuffer_WithOptionalWrap(context, &dfsBuffer, currentElementTreeNode, currentElement, scrollOffset);
            }
        }
